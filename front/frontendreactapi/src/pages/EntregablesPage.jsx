...
import { fetchUsuarios, crearUsuario, actualizarUsuarioPorId, eliminarUsuarioPorId } from '../api/entregables';
...
const UsuarioRow = memo(({ usuario, onEdit, onDelete }) => (
    ...
    <tr>
        ...
        <td class="d-grid gap-2 d-md-flex justify-content-md-end">
            ...
            <button class="btn btn-danger" onClick={() => onDelete(usuario.IdEntregable)}>Eliminar</button>
        </td>
    </tr>
));
...
function EstadosPage() {
            ...
            try {
                if (!esEdicion) {
                    let newProyect = {
                        ...
                        FechaEntrega: usuarioActual.FechaEntrega,
                        Descripcion: usuarioActual.Descripcion,
                    }
                    await crearUsuario(window.location.pathname,newProyect);
                    setMensaje('Entregables creado correctamente.');
                } else {
                    await actualizarUsuarioPorId(window.location.pathname,usuarioActual.IdEntregable, usuarioActual);
                    setMensaje('Entregables actualizado correctamente.');
                }
    ...
    return (<div style={{margin:"10px"}}> <h1>Gestión de Entregables</h1>
        {cargando && <p>Cargando...</p>}
        {mensaje && <div>{mensaje}</div>}
        <table className="table table-striped caption-top">
            <caption>Lista de Entregable</caption>
            <thead>
                <tr>
                    <th>Nombre</th>
        ...
        <form onSubmit={guardarUsuario}>
            ...
            <div class="mb-3">
                <label for="exampleInputFechaEntrega1" class="form-label">Fecha de entrega:</label>
                <input type="date" class="form-control" name="FechaEntrega" value={usuarioActual.FechaEntrega} onChange={manejarCambio} />
            </div>
            <div class="mb-3">
