...
import { fetchUsuarios, crearUsuario, actualizarUsuarioPorId, eliminarUsuarioPorId } from '../api/tipoProyectos';
...
const UsuarioRow = memo(({ usuario, onEdit, onDelete }) => (
    ...
    <tr>
        ...
        <td class="d-grid gap-2 d-md-flex justify-content-md-end">
            ...
            <button class="btn btn-danger" onClick={() => onDelete(usuario.IdTipoProyecto)}>Eliminar</button>
        </td>
    </tr>
));
...
function TipoProyectoPage() {
        ...
        async (e) => {
            ...
            try {
                if (!esEdicion) {
                    ...
                    setMensaje('tipo de Proyecto creado correctamente.');
                } else {
                    await actualizarUsuarioPorId(window.location.pathname,usuarioActual.IdTipoProyecto, usuarioActual);
                    setMensaje('tipo de Proyecto actualizado correctamente.');
                }
    ...
    const eliminarUsuario = useCallback(
        async (Id) => {
            const confirmado = window.confirm(`¿Seguro que quieres eliminar al Tipo de Proyecto?`);
            if (!confirmado) return;
            try {
                await eliminarUsuarioPorId(window.location.pathname,Id);
                setMensaje('tipo de Proyecto eliminado correctamente.');
                await cargarUsuarios();
            } catch (err) {
                setMensaje(`Error: ${err.message}`);
            }
    ...
    const filasUsuarios = useMemo(
        () =>
            usuarios.map((u) => (<UsuarioRow key={u.IdTipoProyecto} usuario={u} onEdit={prepararEdicion} onDelete={eliminarUsuario} />
            )),
        [usuarios, prepararEdicion, eliminarUsuario]
    );

    return (<div style={{margin:"10px"}}> <h1>Gestión Tipos De Proyectos</h1>
        {cargando && <p>Cargando...</p>}
        {mensaje && <div>{mensaje}</div>}
        <table className="table table-striped caption-top">
            <caption>Lista de Tipos De Proyectos</caption>
            <thead>
                <tr>
                    <th>Nombre</th>
                    <th>Descripción</th>
