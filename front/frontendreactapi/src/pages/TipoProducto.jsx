...
function TipoProducto() {
    ...
    const cargarEstado = useCallback(async () => {
        ...
        setCargando(true);
        try {
            const data = await get('/TipoProducto', token);
    ...
    const manejarEnvio = useCallback( async (e) => {
        ...
        try {
            if (!esEdicion) { 
                await post('/TipoProducto',EstadoActual, token);
                setMensaje('TipoProducto creado correctamente.');
            } else {
                await put('/TipoProducto',EstadoActual.Id, EstadoActual,null, token);
                setMensaje('TipoProducto actualizado correctamente.');
            }
            await cargarEstado();
            limpiarFormulario();
        } catch (err) {
    ...
    const manejarEliminacion = useCallback(async (Id, token) => {
        const confirmado = window.confirm(`¿Seguro que quieres eliminar el Tipo Producto?`);
        if (!confirmado) return;
        try {
            await del('/TipoProducto',Id, token);
            setMensaje('Tipo Producto eliminado correctamente.');
            await cargarEstado();
        } catch (err) {
            setMensaje(`Error: ${err.message}`);
        }
        ...
        <div className="container">
            ...
            <form onSubmit={manejarEnvio}>
                ...
                <button type="submit" className="btn btn-primary">
                    {esEdicion ? 'Actualizar Tipo Producto' : 'Crear Tipo Producto'}
                </button>
            </form>
            {cargando ? (
...
export default memo(TipoProducto);