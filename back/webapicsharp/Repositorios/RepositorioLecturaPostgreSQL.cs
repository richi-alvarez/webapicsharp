// RepositorioLecturaPostgreSQL.cs — Implementación concreta para leer datos usando ADO.NET y Postgres
// Ubicación: Repositorios/RepositorioLecturaPostgreSQL.cs
//
// Principios SOLID aplicados:
// - SRP: Esta clase solo se encarga de leer datos desde Postgres, nada más
...
using Npgsql;                                          // Para NpgsqlConnection, NpgsqlCommand, NpgsqlParameter
...
{
    /// <summary>
    /// Implementación específica para leer datos de Postgres usando ADO.NET.
    /// 
    /// Esta clase encapsula toda la lógica específica de Postgres:
    /// - Sintaxis SQL específica (TOP en lugar de LIMIT, corchetes para escapar nombres)
    /// - Tipos de datos específicos de Postgres y su conversión a tipos .NET
    /// - Manejo de conexiones NpgsqlConnection y NpgsqlCommand
    /// - Esquema por defecto "public" característico de Postgres
    /// - Manejo específico de excepciones NpgsqlException
    /// 
    /// ¿Por qué una clase específica para Postgres?
    /// Cada proveedor de base de datos tiene particularidades:
    /// - Postgres: TOP, corchetes [], esquema "public", NpgsqlConnection
    /// - PostgreSQL: LIMIT, comillas "", esquema "public", NpgNpgsqlConnection
    /// - MariaDB: LIMIT, backticks ``, sin esquemas, MyNpgsqlConnection
    /// 
    /// Cumple estrictamente SRP: solo acceso a datos de Postgres.
    ...
    public class RepositorioLecturaPostgreSQL : IRepositorioLecturaTabla
    {
        ...
        /// 1. Program.cs registra: AddScoped<IRepositorioLecturaTabla, RepositorioLecturaPostgreSQL>()
        ...
        ///    - Crea nueva instancia de RepositorioLecturaPostgreSQL (porque es Scoped)
        ...
        /// <summary>
        /// Implementa IRepositorioLecturaTabla.ObtenerFilasAsync para Postgres.
        /// 
        /// Este método es el corazón de la clase: toma parámetros genéricos y los convierte
        /// en una consulta SQL específica de Postgres, ejecuta la consulta, y devuelve
        /// los resultados en el formato estándar Dictionary<string, object?>.
        /// 
        /// Proceso específico para Postgres:
        /// 1. Validar parámetros de entrada según reglas de negocio
        /// 2. Aplicar valores por defecto específicos de Postgres (esquema="public", limite=1000)
        /// 3. Construir consulta SQL con sintaxis específica de Postgres (TOP, corchetes)
        /// 4. Obtener cadena de conexión vía IProveedorConexion (aplicando DIP)
        /// 5. Crear y ejecutar NpgsqlConnection/NpgsqlCommand de forma asíncrona
        /// 6. Procesar SqlDataReader y mapear resultados a Dictionary<string, object?>
        /// 7. Manejar errores específicos de Postgres con contexto apropiado
        ...
        /// Nombre de la tabla a consultar en Postgres. Obligatorio y no puede estar vacío.
        ...
        /// </param>
        /// <param name="esquema">
        /// Esquema de la tabla en Postgres. Si es null o vacío, usa "public" por defecto.
        /// Se escapa automáticamente con corchetes.
        /// Ejemplos: "public" (por defecto), "ventas", "inventario", "rrhh"
        /// </param>
        /// <param name="limite">
        ...
        /// - Clave: nombre de columna tal como está definido en Postgres
        /// - Valor: dato de la columna, con DBNull.Value convertido a null C#
        /// 
        /// Conversión de tipos Postgres → .NET:
        ...
        /// - No se puede conectar a Postgres (servidor caído, red, etc.)
        ...
        public async Task<IReadOnlyList<Dictionary<string, object?>>> ObtenerFilasAsync(
            ...
            string? esquema,       // Esquema de Postgres (opcional, por defecto "public")
        ...
        )
        {
            ...
            // Aplicar valores por defecto específicos de Postgres
            // "public" es el esquema por defecto en Postgres (Database Owner)
            ...
            // ==============================================================================
            // FASE 3: CONSTRUCCIÓN DE CONSULTA SQL ESPECÍFICA DE Postgres
            // ==============================================================================

            // Construir consulta SQL con sintaxis específica de Postgres:
            // - TOP (limiteFinal) en lugar de LIMIT (usado por PostgreSQL/Postgres)
            // - Corchetes [] para escapar nombres de esquema/tabla (protege contra palabras reservadas)
            // - Formato [esquema].[tabla] característico de Postgres
            string consultaSql = $"SELECT * FROM \"{esquemaFinal}\".\"{nombreTabla}\" LIMIT {limiteFinal}";
            ...
            // - Son el estándar de Postgres (PostgreSQL usa "", Postgres usa ``)
            ...
            try
            {
                ...
                // ==============================================================================
                // FASE 6: CONEXIÓN A Postgres
                // ==============================================================================

                // Crear conexión específica de Postgres usando Microsoft.Data.SqlClient
                ...
                // NpgsqlCommand encapsula la consulta SQL y parámetros (si los hubiera)
                ...
                while (await lector.ReadAsync())
                {
                    ...
                    {
                        // Obtener nombre de columna tal como está definido en Postgres
                        ...
                        // Conversión crítica: DBNull.Value (Postgres) → null (C#)
                        // Postgres usa DBNull.Value para representar valores NULL
            ...
            {
                // ==============================================================================
                // MANEJO ESPECÍFICO DE ERRORES DE PostgreSQL CON FALLBACK A ESQUEMA POR DEFECTO
                // ==============================================================================

                // Postgres usa códigos de error estándar (SQLSTATE), por ejemplo:
                // - 42P01: Undefined table (tabla o esquema no existe)
                // - 28P01: Invalid password
                // - 57014: Query canceled (timeout)

                // LÓGICA DE FALLBACK: Si esquema no existe, intentar con "public"
                // 42P01 significa "Undefined table"
                if (excepcionPg.SqlState == "42P01" &&
                    !string.IsNullOrWhiteSpace(esquema) &&
                    !esquema.Equals("public", StringComparison.OrdinalIgnoreCase))
                {
                    try
                    {
                        // Reintentar automáticamente con esquema por defecto "public"
                        return await ObtenerFilasAsync(nombreTabla, "public", limite);
                    }
                    catch
                    {
                        throw new InvalidOperationException(
                            $"Error PostgreSQL: La tabla '{esquema}.{nombreTabla}' no existe en el esquema especificado. " +
                            $"Se intentó automáticamente con 'public.{nombreTabla}' pero tampoco existe. " +
                            $"Verifique la existencia de la tabla y el esquema.",
                            excepcionPg
                        );
                    }
                }

                // ERROR NORMAL - No relacionado con esquema incorrecto
                throw new InvalidOperationException(
                    $"Error de PostgreSQL al consultar '{esquemaFinal}.{nombreTabla}': {excepcionPg.MessageText}. " +
                    $"Código SQLSTATE: {excepcionPg.SqlState}.",
                    excepcionPg
                );
            }
            catch (Exception excepcionGeneral)
            {
                ...
                // Capturar cualquier otro error que no sea específico de Postgres
                // Ejemplos: errores de red, OutOfMemoryException, etc.
                throw new InvalidOperationException(
                    $"Error inesperado al acceder a Postgres para tabla '{esquemaFinal}.{nombreTabla}': {excepcionGeneral.Message}. " +
        ...
        /// <summary>
        /// Implementa la consulta filtrada por clave específica para Postgres.
        /// 
        ...
        /// - Usa NpgsqlParameter para prevenir inyección SQL
        ...
        /// <param name="esquema">Esquema de la tabla (null = "public" por defecto)</param>
        ...
        )
        {
            ...
            // Aplicar esquema por defecto de Postgres si no se especifica
            string esquemaFinal = string.IsNullOrWhiteSpace(esquema) ? "public" : esquema.Trim();
            ...
            // BIEN: "WHERE columna = @valor"           ← Seguro con NpgsqlParameter
            ...
            {
                // ==============================================================================
                // FASE 5: CONEXIÓN A Postgres (IGUAL QUE ObtenerFilasAsync)
                // ==============================================================================
                ...
                // NpgsqlParameter maneja automáticamente:
                ...
                while (await lector.ReadAsync())
                {
                    ...
                    for (int indiceColumna = 0; indiceColumna < lector.FieldCount; indiceColumna++)
                    {
                        // Obtener nombre de columna tal como está en Postgres
                        string nombreColumna = lector.GetName(indiceColumna);

                        // Convertir DBNull.Value (Postgres) a null (C#)
                        object? valorColumna = lector.IsDBNull(indiceColumna)
                            ? null
                ...
                // - SqlDataReader, NpgsqlCommand, NpgsqlConnection
            }
            catch (NpgsqlException excepcionSql)
            {
                // ==============================================================================
        ...
        )
        {
            ...
            try
            {
                ...
                {
                    // Convertir null de C# a DBNull.Value de Postgres
            ...
            {
                // Manejo específico de errores de PostgreSQL con contexto de inserción
                throw new InvalidOperationException(
                    $"Error PostgreSQL al insertar en tabla '{esquemaFinal}.{nombreTabla}': {excepcionPg.MessageText}. " +
                    $"Código SQLSTATE: {excepcionPg.SqlState}. " +
        ...
        /// <summary>
        /// Implementa la actualización de registro para Postgres con soporte para encriptación BCrypt.
        ...
        )
        {
            ...
            // Generar la cláusula SET con comillas dobles para PostgreSQL
        ...
        /// <summary>
        /// Implementa la eliminación de registro para Postgres.
        ...
        /// <summary>
        /// Implementa la obtención de hash de contraseña para Postgres.
        ...
        // otras posibles operaciones específicas de Postgres se pueden agregar aquí
...
//    - Define CÓMO leer datos específicamente de Postgres
...
//    - Solo se encarga de acceso a datos de Postgres (una responsabilidad)
...
//
// 4. CARACTERÍSTICAS ESPECÍFICAS DE Postgres:
//    - Esquema por defecto: "public" (Database Owner)
//    - Sintaxis de límite: TOP (no LIMIT como PostgreSQL/Postgres)
//    - Escapado de nombres: corchetes [] (no comillas "" como PostgreSQL)
//    - Conversión de tipos: DBNull.Value → null
//    - Excepciones específicas: NpgsqlException con códigos numéricos
//
...
//    - Registrar en Program.cs: AddScoped<IRepositorioLecturaTabla, RepositorioLecturaPostgreSQL>
...
//    var repo = new RepositorioLecturaPostgreSQL(mockProveedor.Object);
