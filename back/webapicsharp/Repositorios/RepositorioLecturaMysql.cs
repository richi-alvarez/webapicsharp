// RepositorioLecturaMysql.cs — Implementación concreta para leer datos usando ADO.NET y Mysql
// Ubicación: Repositorios/RepositorioLecturaMysql.cs
//
...
using MySqlConnector;                                  // Para MySqlConnection, MySqlCommand, MySqlParameter
...
{
    /// <summary>
    /// Implementación específica para leer datos de Mysql usando ADO.NET.
    /// 
    /// Esta clase encapsula toda la lógica específica de Mysql:
    /// - Sintaxis SQL específica (TOP en lugar de LIMIT, corchetes para escapar nombres)
    /// - Tipos de datos específicos de Mysql y su conversión a tipos .NET
    /// - Manejo de conexiones MySqlConnection y MySqlCommand
    /// - Esquema por defecto "dbo" característico de Mysql
    /// - Manejo específico de excepciones MySqlException
    /// 
    /// ¿Por qué una clase específica para Mysql?
    /// Cada proveedor de base de datos tiene particularidades:
    /// - Mysql: TOP, corchetes [], esquema "dbo", MySqlConnection
    /// - PostgreSQL: LIMIT, comillas "", esquema "public", NpgMysqlConnection
    /// - MariaDB: LIMIT, backticks ``, sin esquemas, MyMySqlConnection
    /// 
    /// Cumple estrictamente SRP: solo acceso a datos de Mysql.
    /// No contiene validaciones de negocio ni lógica de presentación.
    /// </summary>
    public class RepositorioLecturaMysql : IRepositorioLecturaTabla
    {
        ...
        /// 1. Program.cs registra: AddScoped<IRepositorioLecturaTabla, RepositorioLecturaMysql>()
        ...
        ///    - Crea nueva instancia de RepositorioLecturaMysql (porque es Scoped)
        ...
        /// <summary>
        /// Implementa IRepositorioLecturaTabla.ObtenerFilasAsync para Mysql.
        /// 
        /// Este método es el corazón de la clase: toma parámetros genéricos y los convierte
        /// en una consulta SQL específica de Mysql, ejecuta la consulta, y devuelve
        /// los resultados en el formato estándar Dictionary<string, object?>.
        /// 
        /// Proceso específico para Mysql:
        /// 1. Validar parámetros de entrada según reglas de negocio
        /// 2. Aplicar valores por defecto específicos de Mysql (esquema="dbo", limite=1000)
        /// 3. Construir consulta SQL con sintaxis específica de Mysql (TOP, corchetes)
        /// 4. Obtener cadena de conexión vía IProveedorConexion (aplicando DIP)
        /// 5. Crear y ejecutar MySqlConnection/MySqlCommand de forma asíncrona
        /// 6. Procesar SqlDataReader y mapear resultados a Dictionary<string, object?>
        /// 7. Manejar errores específicos de Mysql con contexto apropiado
        ...
        /// Nombre de la tabla a consultar en Mysql. Obligatorio y no puede estar vacío.
        ...
        /// </param>
        /// <param name="esquema">
        /// Esquema de la tabla en Mysql. Si es null o vacío, usa "dbo" por defecto.
        ...
        /// - Clave: nombre de columna tal como está definido en Mysql
        /// - Valor: dato de la columna, con DBNull.Value convertido a null C#
        /// 
        /// Conversión de tipos Mysql → .NET:
        ...
        /// - No se puede conectar a Mysql (servidor caído, red, etc.)
        ...
        public async Task<IReadOnlyList<Dictionary<string, object?>>> ObtenerFilasAsync(
            ...
            string? esquema,       // Esquema de Mysql (opcional, por defecto "dbo")
        ...
        )
        {
            ...
            // ==============================================================================
            // FASE 3: CONSTRUCCIÓN DE CONSULTA SQL ESPECÍFICA DE Mysql
            // ==============================================================================

            // Construir consulta SQL con sintaxis específica de Mysql:
            ...
            // - Formato [esquema].[tabla] característico de Mysql
            string consultaSql = $"SELECT * FROM {nombreTabla} LIMIT {limiteFinal}";
            ...
            // - Son el estándar de Mysql (PostgreSQL usa "", MySQL usa ``)
            ...
            try
            {
                ...
                // ==============================================================================
                // FASE 6: CONEXIÓN A Mysql
                // ==============================================================================

                // Crear conexión específica de Mysql usando Microsoft.Data.SqlClient
                ...
                // MySqlCommand encapsula la consulta SQL y parámetros (si los hubiera)
                ...
                while (await lector.ReadAsync())
                {
                    ...
                    {
                        // Obtener nombre de columna tal como está definido en Mysql
                        ...
                        // Conversión crítica: DBNull.Value (Mysql) → null (C#)
                        // Mysql usa DBNull.Value para representar valores NULL
            ...
            {
                // ==============================================================================
                // MANEJO ESPECÍFICO DE ERRORES DE Mysql CON FALLBACK A ESQUEMA POR DEFECTO
                // ==============================================================================

                // MySqlException contiene códigos de error específicos de Mysql:
                ...
                throw new InvalidOperationException(
                    $"Error de Mysql al consultar la tabla '{nombreTabla}': {excepcionSql.Message}. " +
                    $"Código de error Mysql: {excepcionSql.Number}. " +
            ...
            catch (Exception excepcionGeneral)
            {
                ...
                // Capturar cualquier otro error que no sea específico de Mysql
                // Ejemplos: errores de red, OutOfMemoryException, etc.
                throw new InvalidOperationException(
                    $"Error inesperado al acceder a Mysql para tabla '{nombreTabla}': {excepcionGeneral.Message}. " +
        ...
        /// <summary>
        /// Implementa la consulta filtrada por clave específica para Mysql.
        /// 
        ...
        /// - Usa MySqlParameter para prevenir inyección SQL
        ...
        )
        {
            ...
            // BIEN: "WHERE columna = @valor"           ← Seguro con MySqlParameter
            string consultaSql = $"SELECT * FROM {nombreTabla} WHERE {nombreClave} = @valor";
            ...
            {
                // ==============================================================================
                // FASE 5: CONEXIÓN A Mysql (IGUAL QUE ObtenerFilasAsync)
                // ==============================================================================
                ...
                // MySqlParameter maneja automáticamente:
                ...
                while (await lector.ReadAsync())
                {
                    ...
                    for (int indiceColumna = 0; indiceColumna < lector.FieldCount; indiceColumna++)
                    {
                        // Obtener nombre de columna tal como está en Mysql
                        string nombreColumna = lector.GetName(indiceColumna);

                        // Convertir DBNull.Value (Mysql) a null (C#)
                        object? valorColumna = lector.IsDBNull(indiceColumna)
                            ? null
                ...
                // - SqlDataReader, MySqlCommand, MySqlConnection
            }
            catch (MySqlException excepcionSql)
            {
                // ==============================================================================
                ...
                throw new InvalidOperationException(
                    $"Error SQL al filtrar tabla '{nombreTabla}' por columna '{nombreClave}' con valor '{valor}': {excepcionSql.Message}. " +
            ...
            catch (Exception excepcionGeneral)
            {
                ...
                throw new InvalidOperationException(
                    $"Error inesperado al filtrar tabla '{nombreTabla}' por {nombreClave}='{valor}': {excepcionGeneral.Message}",
                    excepcionGeneral
                );
            }
        ...
        )
        {
            ...
            string consultaSql = $"INSERT INTO {nombreTabla} ({columnas}) VALUES ({parametros})";
            try
            {
                // ==============================================================================
                ...
                {
                    // Convertir null de C# a DBNull.Value de Mysql
            ...
            {
                // Manejo específico de errores de Mysql con contexto de inserción
                throw new InvalidOperationException(
                    $"Error SQL al insertar en tabla '{nombreTabla}': {excepcionSql.Message}. " +
            ...
            {
                throw new InvalidOperationException(
                    $"Error inesperado al insertar en '{nombreTabla}': {excepcionGeneral.Message}",
                    excepcionGeneral
                );
            }
        }
        ...
        /// <summary>
        /// Implementa la actualización de registro para Mysql con soporte para encriptación BCrypt.
        ...
        )
        {
            ...
            string consultaSql = $"UPDATE {nombreTabla} SET {clausulaSet} WHERE `{nombreClave}` = @valorClave";
            ...
            {
                // Manejo específico de errores de Mysql con contexto de actualización
                throw new InvalidOperationException(
                    $"Error SQL al actualizar tabla '{nombreTabla}' WHERE {nombreClave}='{valorClave}': {excepcionSql.Message}. " +
                    $"Código de error: {excepcionSql.Number}. " +
            ...
            {
                throw new InvalidOperationException(
                    $"Error inesperado al actualizar '{nombreTabla}' WHERE {nombreClave}='{valorClave}': {excepcionGeneral.Message}",
                    excepcionGeneral
                );
            }
        }
        ...
        /// <summary>
        /// Implementa la eliminación de registro para Mysql.
        ...
        )
        {
            ...
            string consultaSql = $"DELETE FROM `{nombreTabla}` WHERE `{nombreClave}` = @valorClave;";
            ...
            {
                // Manejo específico de errores de Mysql con contexto de eliminación
                throw new InvalidOperationException(
                    $"Error SQL al eliminar de tabla '{nombreTabla}' WHERE {nombreClave}='{valorClave}': {excepcionSql.Message}. " +
                    $"Código de error: {excepcionSql.Number}. " +
            ...
            {
                throw new InvalidOperationException(
                    $"Error inesperado al eliminar de '{nombreTabla}' WHERE {nombreClave}='{valorClave}': {excepcionGeneral.Message}",
                    excepcionGeneral
                );
            }
        }
        /// <summary>
        /// Implementa la obtención de hash de contraseña para Mysql.
        ...
        )
        {
            ...
            catch (MySqlException excepcionSql)
            {
                ...
                throw new InvalidOperationException(
                    $"Error SQL al obtener hash de contraseña de tabla '{nombreTabla}' WHERE {campoUsuario}='{valorUsuario}': {excepcionSql.Message}. " +
                    $"Código de error: {excepcionSql.Number}",
                    excepcionSql
                );
            }
            ...
            {
                throw new InvalidOperationException(
                    $"Error inesperado al obtener hash de contraseña de '{nombreTabla}': {excepcionGeneral.Message}",
                    excepcionGeneral
                );
            }
        }
        // otras posibles operaciones específicas de Mysql se pueden agregar aquí
...
//    - Define CÓMO leer datos específicamente de Mysql
...
//    - Solo se encarga de acceso a datos de Mysql (una responsabilidad)
...
//    - Excepciones específicas: MySqlException con códigos numéricos
//
...
//    - Registrar en Program.cs: AddScoped<IRepositorioLecturaTabla, RepositorioLecturaMysql>
...
//    var repo = new RepositorioLecturaMysql(mockProveedor.Object);
